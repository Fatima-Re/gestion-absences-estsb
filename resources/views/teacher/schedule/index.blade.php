            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            <a href="{{ route('teacher.schedule.index', ['week' => $currentWeek - 1]) }}" class="btn btn-outline-secondary btn-sm">← Semaine précédente</a>
                            <a href="{{ route('teacher.schedule.index', ['week' => $currentWeek + 1]) }}" class="btn btn-outline-secondary btn-sm">Semaine suivante →</a>
                        </div>
                    </div>
                </div>
            </div>
                                        ...
                                        <td>
                                            @if($sessions->count() > 0)
                                                @foreach($sessions as $session)
                                                    <div class="p-2 mb-1 border rounded" style="background-color: var(--primary-color-1); color: white; font-size: 0.8em;">
                                                        <strong>{{ $session->module->name }}</strong><br>
                                                        <small>Groupe: {{ $session->group->name }}</small><br>
                                                        ...
                                                        <div class="mt-2">
                                                            @if($session->status === \App\Models\CourseSession::STATUS_SCHEDULED)
                                                                <a href="{{ route('teacher.attendance.create', $session) }}" class="btn btn-sm btn-light">
                                                                    Prendre la présence
                                                                </a>
                                                            @else
                                                                <a href="{{ route('teacher.attendance.show', $session) }}" class="btn btn-sm btn-outline-light">
                                                                    Voir présence
                                                                </a>
                                                            @endif
                                                        </div>
